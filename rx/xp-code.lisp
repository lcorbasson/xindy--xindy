...
(export '(#-clisp formatter copy-pprint-dispatch pprint-dispatch
...
(defun backquote-print (xp list)
  (if (and (consp (cdr list)) (listp (cddr list)) (null (cdddr list)))
      (funcall (formatter "`~W") xp (cadr list))
      (fn-call xp list)))
(defun splice-print (xp list)
  (if (and (consp (cdr list)) (null (cddr list)))
      (let ((form (cadr list)))
        (ecase (car list)
          (sys::splice (funcall (formatter ",@") xp))
          (sys::nsplice (funcall (formatter ",.") xp)))
        (if (and (consp form) (eq (car form) 'sys::unquote)
                 (consp (cdr form)) (null (cddr form)))
            (funcall (formatter "~W") xp (cadr form))
            (funcall (formatter "'~W") xp form)))
      (fn-call xp list)))
(defun unquote-print (xp list)
  (if (and (consp (cdr list)) (null (cddr list)))
      (funcall (formatter ",~W") xp (cadr list))
      (fn-call xp list)))
)
...
#+CLISP
(set-pprint-dispatch+ '(cons (member sys::backquote)) #'backquote-print '(0) *IPD*)
#+CLISP
(set-pprint-dispatch+ '(cons (member sys::splice)) #'splice-print '(0) *IPD*)
#+CLISP
(set-pprint-dispatch+ '(cons (member sys::nsplice)) #'splice-print '(0) *IPD*)
#+CLISP
(set-pprint-dispatch+ '(cons (member sys::unquote)) #'unquote-print '(0) *IPD*)
