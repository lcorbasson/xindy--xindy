...
Wir werden später noch weitere solcher Attribute einführen. Zunächst
...
Stichwortverzeichnis häufig noch Indexe für Autoren, Symbole \etc{}$\,$.
In \cite{griechen} werden beispielsweise insgesamt vier verschiedene
...
noch nach Bedarf unterschiedlich im Layout, können aber aus
prinzipiell einem einzigen Ursprungs-Datenstrom von Indexeinträgen
stammen.  Unser Modell sollte also die Verarbeitung von verschiedenen
...
entspricht, und \emph{run-in intended style}, welcher Substrukturen
...
wird das Problem teilweise gelöst, indem zum Sortierschlüssel
\emph{sort key} ein weiterer Druckschlüssel\remark{Druckschlüssel}
\emph{actual key} eingeführt wird, welcher beim der abschließenden
Ausgabeformatierung den Sortierschlüssel substituiert. Dieses
Verfahren umgeht die \quasi{St.\ Louis}-Problematik, indem man in
einem solchen Fall die Substitution separat angibt, ist aber
prinzipiell sehr unbequem zu handhaben.  Man bedenke, daß in der Regel
viele spezielle nationale Zeichensätze\remark{nationale Alphabete} mit
Umlauten nicht innerhalb des lateinischen Alphabets sortierbar sind.
Nach deutschen Orthographieregeln wird der Buchstabe \quasi{ä} je nach
...
Eine Möglichkeit den Index zu erweitern besteht in der Hinzunahme von
\emph{Permutierten Indexschlüsseln}\remark{Permutierter
...
Nachdem wir uns einen Überlick über die Stichwörter verschafft haben,
gehen wir nun dazu über, die Lokationen in vertiefter Weise zu
...
Dokument verwiesen werden soll ist insbesondere eine Frage, ob auf die
interne oder externe Struktur oder beides verwiesen werden soll.
...
wie \quasi{11a} erweitert, um bei nachträglichen Korrekturen die
...
die zugehörigen Paragraphen aufgezählt. Als Beispiel soll der folgende
Auszug dienen.\footnote{Fette Nummern bezeichnen hier die Nummer des
  Gesetzbuches, während die normalen Nummern auf die zugehörigen
...
wie Bindestriche oder Trennpunkte zwischen den Hierarchien auftreten
...
--- wie man aus dem Datenflußdiagramm~\ref{fig:datenfluss} erkennen
kann --- nichts über die Internas des Textsatzsystems weiß, muß unser
System in der Lage sein, aus Zeichenfolgen mittels der Angaben im
Indexstyle die Lokationsreferenzen zu erkennen. Wir bezeichnen das der
Indexverarbeitung zugrundeliegende Alphabet von Zeichen als das
\emph{Dokumentalphabet}\remark{Dokument\-alphabet}. Dieses
Alphabet ist der gemeinsame Zeichenvorrat des Satzsystems und des
...
liegen also im Format dieses Dokumentalphabets vor und das Indexsystem
muß daraus die Lokationsreferenzen in ihre Struktur zerlegen.
...
\Dalpha{} vom Typ ISO-Latin, so ist die Steiennummer \quasi{13} ein
...
\Dalpha{sn} $= \{\mq{0}\ldots\mq{9}\}$\footnote{also die der Zif"|fern
  des arabischen Zahlensystems} des Dokumentalphabets \Dalpha{}. Die
Menge aller Worte über \Dalpha{sn} bilden nach
...
\mq{A},\ \Salpha{1}{2} $=$ \mq{B} \ldots{} \Salpha{1}{26} $=$ \mq{Z}.
Daraus ergibt sich das Basisalphabet
...
\begin{ttable}%
  ...
  \begin{tabular}{|l|l|c|c|}
    ...
    Indexaufzählung & $x_{1}, x_{2},\ldots, x_{9},\ldots$ &
    $\{x, 0\ldots9\}$ &\\
    ...
    griechisch (klein) & $\alpha, \beta, \gamma, \delta,\ldots, \omega$
    & $\{\alpha\ldots\omega\}$ & \\
...
das arabische und römische Zif"|fernfolgen ist diese Abbildung bereits
durch das üblicherweise benutzte Zahlensystem gegeben.\footnote{Man
  beachte, daß die \oa \quasi{Zahlen} zunächt einmal nur Zeichenfolgen
...
belegen können. Hierbei muß dann ebenfalls eine geeignete
Ordnungsrelation definiert werden. Dies könnte durch eine einfache
Aufzählung der Symbole geschehen wie in der letzten Zeile der Tabelle
dargestellt. Dort sind die Worte \quasi{Kapitel} und \quasi{Anhang}
Symbole des Symbolalphabets, denen aufgrund der Aufzählungsreihenfolge
...
\item \emph{Varklassen},\remark{Varklassen} deren Struktur sich
  ...
  Gliederungsstruktur an. Man kann hier auch argumentieren, daß jede
...
\item \Strukzwei{\texttt{num}}{\textbf{.}}{\texttt{num}}
  ...
  definiert eine Lokationsklasse, deren erste und zweite Hierarchiestufe
  durch arabische Zahlen repräsentiert werden. Gültige Repräsentanten
  dieser Klasse sind beispielsweise die folgenden, aufsteigend sortierten
  Referenzen 1.1, 1.2, 1.3 \ldots{} 2.0, 2.1 \ldots{} 2.20 \ldots{} .
  ...
  den Zählebenen unterschieden werden kann (12 kann sein 1.2 oder
  \textsl{zwölf\/}).
...
\begin{Lem} Zwei Sortierungsalphabete ${\cal A}$ und ${\cal B}$ sind
  grundsätzlich miteinander \emph{verträglich}, wenn für deren
...
Komponententypen und Trennzeichen einer Lokationsklasse passen muß, um
...
Zuordnungsstrategie Exaktes Zuordnen für den Präfix-Anteil und die
...
\noindent Die Spalte \emph{eindeutig} bezeichnet Referenzen, welche
...
Spalte \emph{mehrdeutig} können nicht eindeutig einer einzigen Klasse
zugeordnet werden.
...
Da der Leser die korrekte Zuordnung verstehen muß, treten solche Fälle
in der Praxis normalerweise nicht auf. Sie sollten deshalb von Autoren
auch nicht zur Dokumentstrukturierung verwendet werden. Man muß in
diesem Fall davon ausgehen, daß die Angabe der Lokationsklassen
unvollständig ist und der Benutzer korrigierend eingreifen sollte.
Eine Korrektur kann darin bestehen, bestimmte Klassen noch mit
geeigneten Dummy-Präfixen zu versehen. Für die Lokationsklassen
...
Textsatzsystem mit separat Darstellungsformen wie \zB \emph{normal},
\emph{fett} und \emph{kursiv} verknüpft werden.  Weiterhin sollen
solche unterschiedlich attributierten Lokationsreferenzen
unterschiedlich in die Lokationsliste einsortiert werden. Wir ordnen
daher einer Lokationsreferenz ein Attribut zu, welches für
verschiedenartige Kategorisierungszwecke verwendet werden kann.  Wir
verwenden hierfür den Begriff
...
\subsection*{Ergebnisser der strukturellen Analyse}
...
\end{center}
%
so müssen wir in der Lage sein, eine Sortierreihenfolge festzulegen.
...
\begin{Def}
  ...
  $L$ mit der Struktur $S_k = ([s_1]\ldots{}[s_n])$ mit $s_k$
  Sortieralphabet ist für paarweise Lokationsreferenzen $l^1,l^2\in L$
  mit $l^k = ([l_1]\ldots{}[l_n])$ definiert als eine Relation
  \TotOrdLokRefLe für die gilt:
  %
...
verankert. Bedenken wir den Fall von eingefügten Seiten der Varklasse
\Metaclass{\Strukzwei{\texttt{num}}{}{\texttt{alpha}}}, wie sie oft in
Handbüchern nach Änderungen vorkommen, so wissen wir nicht, ob auf die
Seite 11 nun die Seite 12 oder die Seite 11a folgt.  Dieses ist
\emph{Dokumentwissen}\remark{Dokumentwissen}, welches wir unserem
...
\begin{Def}
  Ein \emph{Bereich} ist ein Paar
  \begin{center}
    $\range = ( \locref_1, \locref_2 )$
  \end{center}
  von Lokationsreferenzen gleicher Klasse, für die gilt: \
  $\locref_1\,\TotOrdLokRefLe\,\locref_2$. Die beiden
  Lokationsreferenzen bilden die untere und obere Grenze des Bereichs.
  Wie bereits eingeführt notieren wir Bereiche in der Form
  \Range{x}{y}.
\end{Def}
...
\begin{Def}
  ...
  \begin{tabular}{l@{ }l@{= }lll}
    ...
    3.& \joinrange{$l_1$, $P$} & \Range{$l_1$}{$r_2$} & falls
    \Succp({$l_1,p_1$}).\\%%
...
\end{Def}
%
Regel~1 trifft keine Aussage darüber, ab wievielen aufeinander
folgenden Nachfolgern eine Bereichsbildung zulässig ist (hier:
2~Nachfolger).

Wir lassen bei der Bereichsbildung nur Lokationsreferenzen mit
...
Mischvorgänge innerhalb einer Lokationsklasse fester Struktur.
Anschließend erläutern wir die Modifikationsmöglichkeiten bezüglich
...
\item[separate-sorting]\mbox{}\\ Wir separieren in der Ausgabe alle
  ...
  Wir notieren eine solche Reihenfolge mit einer Aufzählung der
...
\item[mixed-sorting]\mbox{}\\ Wir fassen alle Vertreter bestimmter
  ...
  \textsf{separate-sorting}-Aufzählung enthalten sein. Des weiteren
  definieren wir innerhalb einer solchen Aufzählung eine Rangordnung,
  die die Wichtigkeit der Attribute festlegt. Wir notieren solche
  virtuellen Attribute, indem wir in der Aufzählung der
  ...
  \emph{Verdrängens} widerspiegelt. In unserem Beispiel wird in
...
erhalten wir einen listenförmige Ausgabe. In allen Fällen können die
in Klammern dargestellten Lokationsreferenzen auf Wunsch auch
unterdrückt werden, da sie doppelt in der Referenzliste auftreten.
Dazu besteht die Möglichkeit einer weiteren Wahlmöglichkeit, die wir
...
Mit einer \emph{einfachen} Bereichsbildung werden aufeinander folgene
...
Lokationsreferemz initial zugeordnet wurde, noch weitere
...
\Braces{\textbf{15};\texttt{bold}a} eine Referenz
...
\end{center}
eines Artikels. Wir haben nun folgende Möglichkeiten zur Auswahl:
%
...
Nachfolgerfunktion für Elemente der untersten Ebene.\footnote{Also
  \Succ$(3.1.2,3.1.3)$; aber nicht \Succ$(3.1,3.2)$.} Für Varklassen mit
variabler Länge der Lokationsstruktur kann somit keine implizite
Nachfolgerbestimmung durchgeführt werden. Dies entspricht der
intuitiven Vorstellung eines Nachfolgers, denn wir können ohne
...
benötigen wir eine Möglichkeit bei einer erfolgreichen Zusammenfassung
einer höheren Ebene niedrigere Ebenen zu überdecken (Fälle~2 und 3).
...
Ausnahmefall. Die hier eingeführte Lokationsmatrix bietet eine
