...
a location class of page numbers which has to optically distinguish
...
intended meaning. The attribute values can be accessed during tree
traversal time.
...
the functionality offered by both systems is not absolutely necessary
due to the limited context of an index.

Hence, we aimed for a more efficient solution to the markup problem.
As a first restriction we decided to make available only a subset of
the elements along the path as a node's context. Furthermore we
observed that in many cases there is a need for making the attributes
of an instance accessible as context information.
