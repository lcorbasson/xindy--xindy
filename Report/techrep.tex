...

{
  ...
  \begin{abstract}
    ...
    theoretical part of our work mainly contributes to a formalisation
...
book.\footnote{Especially as it can be used to express feelings (as
  shown by the quotation of \textsc{D. E. Knuth} shows), as well as
  humour (see under \emph{kludges} in~\cite{Steele:common-lisp-2}).}
...
raises two basic questions: (a) how do we recognise that two keywords
...
The problem ob equality is solved with the merge mapping. Two keywords
...
key with a list of all letter groups declared in the index
style.\footnote{This model assumes that letter groups are built using
  a prefix of the sort key, since we know of no other grouping
  mechanism, yet.}
...
\end{description}
%%
The different structures listed above allow a characterisation and
definition of location references which is done with the concept of
\term{location classes}.
...
are actually sequences of elements of \term{finite length}, e.g.\ the
letters \{`A', \ldots{},~`Z'\} or the alphabet \{`\emph{Genesis}',
`\emph{Exodus}', `\emph{Leviticus}'\}. A sequence of finite length can
be declared by simply listing all of its elements, thus
$\alpha_i=\{l_0, l_1, \ldots, l_k\}$ with $l_j$ being the letters of
the alphabet. Each letter is a string. The sorting order inside an
alphabet is simply derived from the definition order ($l_i<l_j
\Leftrightarrow i<j$). An alphabet consisting of only one letter
defines a \emph{separator}, i.e.\ a string that is used to separate
several components of a location reference. Typical examples are
dashes, dots, and blanks (`--', `.', `\verb*| |').
...
exists a sequence $s=(s_1,\ldots{},s_n)$ of strings such that holds
$\forall i.(s_i\in \alpha_i\ \wedge\ L=s_1\ldots{}s_n)$. Using
alphabets to declare the structure of a location class makes it
possible to directly derive an ordinal number for each $s_i$
separately. This enables one to define a natural sorting order on a
complete location class based on the \term{lexicographic order} of the
sequence $s$.

\newpage

\noindent For example, the bible verses can be described in the index
...
a particular page in favour of a definition on the same page. This
...
author may want to let both references coexist next to each other.
...
\end{comment}
This kind of multi-argument dispatching is based directly on the
...
references that can be described as classification, normalisation,
