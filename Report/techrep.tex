...
\begin{zitat}
  ...
  \textsc{Donald E. Knuth}, \textrm{The \TeX{}book}
...
to guide readers in finding the desired information in documents
efficiently and effective. For large documents indexes are
traditionally one form of service that helps finding the desired
information.
...
For textbooks it is necessary that the indexer has sufficient
knowledge of the problem area to make appropriate indexing decisions.
Unfortunately, the index is very often the most neglected part of a
...
currently less flexible than \mkidx. Therefore we have concentrated
mainly on this system. Figure~\ref{fig:data-flow} describes the
typical data flow in the first group of systems. The document is
processed by the \dps{} that extracts the indexing information, the
so-called \term{raw index}. The raw index comes in a system dependent
format containing information about the \term{index entries}. An index
entry typically consists of a \term{keyword} and a \term{location
  reference}. We describe these terms in detail in
section~\ref{sec:index-model}.
%%
{\vertspace=-3ex
\begin{mfigure}
  {Data flow in \dps{}s}
...
adopted to the format of the \dps and the index processor. In \mkidx
the parser is part of the indexing system. It can be configured to
accept a certain range of raw index formats, but essentially the
formats are rather restricted because it was designed primarily as an
index processor for the \TeX-system and its raw index format.

We have removed the parser from the core indexing system and defined a
general raw index specification format. For using our system in
conjunction with some text processing environment, first a parser has
to be implemented that translates the application-specific format into
this format.
...
After the index has completely been processed, its internal
representation must be fed back into the document preparation cycle.
This is done through the \term{markup backend} that outputs the
internal format tagged with appropriate markups defined in the index
style in a representation that can be used by the \dps to typeset the
index.
...
Typically indexes are a collection of ordered keyword each one
...
behaviour. Figure~\ref{fig:adv-index} shows an example index. Despite
the fact that such confusing indexes probably do not exist in
practice, we recognize the two-dimensional structure of an index.
...
In the \term{vertical dimension} the index is structured by the
keywords, since they actually serve as the key for the readers search.
The keywords form a hierarchy which is often visualized by the level
of indentation. This is not always the practice. For example, the
Chicago Manual of Style~\cite{chicago} recommends the so-called
\term{flush-and-hang style} or the \term{run-in indented style} that
are visualizations of the appearance of index hierarchies. As opposed
to hierarchical indexes there is often need for so-called \term{flat}
indexes.

Along the \term{horizontal dimension} we identify at first glance a
list of \term{location references}. A location reference is an object
that serves as a reference to some location inside the document. We
can observe different types of locations in documents: page numbers,
section numbers, parts of the appendix, cross references, etc.\ Most
indexes contain only references to pages but there is a wide
application area in which pages are of less concern.


\subsection{Index Entries}
\label{sec:index-entries}

\enlargethispage{\baselineskip}

Most indexes have a recursive structure since an index entry can
possibly be composed of other index entries. In a tree representation
the index can be seen as a node having index entries as children.
...
``sequential''). A keyword can be interpreted as the \term{path} in
the tree structure to the corresponding location references. This view
...
\item[Section-based Pages.]  References of the form
  ([section]--[page]): \emph{2--8}, \emph{2--12}, \emph{4--7},
  \emph{4--9}.
...
\item[Hierarchical Sections.] References of the form
  ([sect].[sect].[\ldots]): \emph{1.1}, \emph{1.2.1}, \emph{2.1}.
...
\item[Irregular Structures.] References of the form
  ([page]\{[letter]\}): \emph{11}, \emph{11a}, \emph{17}, \emph{17c}.
...
\begin{alltt}
 ...
 (define-location-class "bible-verses"
     ("bible-chapters" :sep "\verb*| |" \footnote{The argument \texttt{:sep} declares the following argument to be a separator.}
      "arabic-numbers" :sep ","
      "arabic-numbers"))
\end{alltt}
%%
This effectively matches all of the examples given above.  The sort
key of the location reference `\emph{Exodus 2, 12}' would then be
...
(2\ts3\ts12).\footnote{Assuming the first element of an alphabet has
  ...
  \emph{2}$\rightarrow$2, and \emph{12}$\rightarrow$12.} Sorting bible
...
Another problem is the potential ambiguity in the here only sketched
matching process. Given the location classes ([arabic][alpha]) and
([arabic][roman]), the following location references match both
classes: \emph{1c}, \emph{2i}, \emph{7m}, since the set of the
lowercase roman numerals $\{c,i,l,m,v,x\}$ and the set of the
lowercase latin letters $\{a,\ldots,z\}$ are not disjoint.  We argue
that this situation is unlikely to occur in practice since the reader
would be confused by such location references, making this a rather
pathological case.\footnote{I hope so, since I actually found no
...
partition the set of all attributes.\footnote{We have not modeled the
  case that an attribute appears in more than one attribute group,
...
%%
\begin{enumerate}[\quad a)]
\item \verb| (define-attributes ( ("definition"   "default") ))|
\item \verb| (define-attributes ( ("definition") ("default") ))|
\end{enumerate}
%%
...
Beyond simply sorting the location references inside an attribute
...
total ordering of the location references using the sort key of the
...
For example, we have page numbers of the form ([arabic]\{[roman]\})
matching the following location references: \emph{11, 11a, 12}. The
corresponding sort keys are (11), (11\ts1) and (12). We can perhaps
infer that (11) could be written as (11\ts0) and then conclude that
\emph{11a} is a successor of \emph{11}, but we actually know nothing
about \emph{11a} and \emph{12}, since we do not know if there exists a
page \emph{11b} in the document. As long as we do not know if there is
another location between \emph{11a} and \emph{12} we cannot build the
range \emph{11--12}, though it might be desirable in the concrete
case.
...
explicitly declares \emph{12} to be a successor of
\emph{11a}---tedious but under certain circumstances acceptable, or
(b) the \dps generates this information automatically and adds it to
the raw index as well. We call this kind of missing information
\term{document knowledge} since it is contained in the document itself
and must be made available to the index processor for entirely solving
...
appear if there is a relation among the attributes not fixed to the
ones appearing in an attribute group. In our case there exists the
virtual location reference [\emph{13}] behaving as a normal
\emph{default} location reference that helps to form the range
\emph{11--15}. If no range can be built, the virtual location
...
for which must hold: $D\subseteq M$.
...
Table~\ref{tab:virtuals} gives examples of possible combinations for
some specific relations. Examples~1--4 characterize the situation in
which the attributes \term{definition} and \emph{default} appear in
two separate attribute groups, whereas in the examples~5--8 both are
in the same attribute group and attribute \emph{definition} also
supersedes attribute \emph{default}. Without this restriction there
would exist even more alternatives. As we can see the additional
relations may help to compress the list of location references under
...
the index style. A cross reference can be assigned an attribute in the
raw index specification format identifying the class of a cross
reference. This class reference can be used to assign specific markup.
This allows for a much more flexible markup and grouping mechanism
than other index processors.
...
avoiding the annoying presence of index entries pointing somewhere
``behind the moon''.
...
\afterpage{%%
  {\vertspace=-2ex%%
    \begin{mfigure}{Tree structure of an index}{fig:index-tree}
      \begin{center}
        \input{index.latex}
      \end{center}
    \end{mfigure}}}
...
solutions to this problem. A next correspondence occurs with the
sorting problem, which in both cases is solved using appropriate
mappings. In the vertical dimension we use a mapping from keywords,
consisting of a list of words over the document alphabet, to another
list in which each element is generated using a injective mapping,
namely the merge and sort mappings. This list is then sorted
lexicographically. The same process appears at the horizontal
dimension, except that we use a list of ordinal numbers to represent
the sort key of a location reference. It is possible for the user to
define arbitrary alphabets along both dimensions using two completely
...
developing an index processor capable of only a subset of our indexing
model will be faced with many problems. The actual implementation is
fairly complex and from our personal opinion not be solvable better
using pipelines. Additionally, we wanted to implement a system that is
usable by many people, not only by experienced
...
part of the core language ready for using. The powerful
macro-mechanism enabled us to implement a system without writing a
dedicated parser. The \Lisp-interpreter is well suited for this
purpose. This has encouraged us to write the production system in
\Lisp as well.
...
\subsection{The Index Style}

As shown in figure~\ref{fig:data-flow} the index processor is
configured with the index style. It understands about 35 different
commands, most of them are used to specify the markup of the final
index. For a detailed description of all commands
see~\cite{xindy:man}.

Since a complete description of an index style can be rather complex
we have adopted a module concept, that enables one to decompose a
complete index style into submodules. This allows the reuse of
components which in practice makes writing a style file an easy task.
We have implemented a set of predefined styles that can be used in an
ad-hoc manner. Though compatibility with \mkidx was not a design goal
the distribution contains tools and modules, that make \xindy mostly
behave as a plug-in for \mkidx.
...
hull over the successor relation of the list defining the attribute
...
index tree it came across from the root to the current node. Starting
from the root-node in figure~\ref{fig:index-tree}, such a path could
look like
%%
\def\emrm#1{\mbox{\normalfont\emph{#1}}}%%
\begin{alltt}
...
\end{alltt}
%%
All markup commands that can be specified in the index style have the
following form:
%%
\def\itarg#1{\mbox{\normalfont\emph{#1}}}
\def\rmarg#1{\mbox{\normalfont #1}}
\begin{alltt}
 (markup-\itarg{index-tree-node-name}
     ...
     \rmarg{[context-dependent options such as \texttt{:attr}, \texttt{:group}, \texttt{:class}, \texttt{:depth}, \ldots]})
\end{alltt}
%%
Since, by default, all fields are optional one can assign the markup
to only a subset of the context arguments. The accessible context
arguments depend on the type of the object that is to be output. For
example, the markup of ranges can be specialized on their length
allowing different markup for ranges of different length. Thus, one
can specify markup of the form \emph{22\,f.} for ranges of length~1,
\emph{31\,ff.} for ranges of length~2, and \emph{11--15} for all other
ranges. We have not made the whole context available to the user, but
...
\end{verbatim}
but redefining the markup for the instances of a certain location
class as follows:
\begin{verbatim}
 (markup-locref :open "\special{" :close "}" :class "special-class")
\end{verbatim}
%%
The event dispatcher must decide which of the defined markup schemes
...
of object-oriented nature. The user declares objects such as
alphabets, constructs location classes, of which location references
are instances. Markup can be attached orthogonally to these objects
using multi-argument dispatching. For users not familiar with this
kind of operation we have added a feature that traces the whole markup
phase so that generated events are shown and the markup is reported
that is emitted in response to these events.
...
dimension, the index entries and their corresponding keywords are
