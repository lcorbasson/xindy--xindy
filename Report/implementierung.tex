...
\item Das System sollte möglichst portabel sein. Die bestehenden
  ...
  sollte möglich sein, das zu entwickelnde System weiterhin in diesem
...
\item Es sollte eine möglichst schnelle Implementierung gestatten, um
  sich auf die wesentlichen Punkte des Systems zu konzentrieren. Aus
  diesem Grund sollten auch fertige Bibliotheken zum Einsatz kommen
...
entsprechende Klassen möglich ist, und es inzwischen eine hohe
Verfügbarkeit dieser Sprache auf vielen unterschiedlichen Plattformen
gibt.
...
F"ur die Rohindexerkennung war urspr"unglich ein eigener Parser
vorgesehen war, ist ein
\texttt{perl}-Skript~\cite{Wall:PP92,Schwartz:LP93} vorgesehen.  Diese
skriptorientierte Programmiersprache bietet vielfältige Möglichkeiten
zur Erkennung von Mustern und verwendet Reguläre Ausdrücke, um die
Indexeinträge zu filtern und in eine Folge von \LISP-Ausdrücken
umzuwandeln. Diese werden dann von \mkxvier-System gelesen und
ausgewertet.
...
\begin{tfigure}{Schematisches Datenmodell der wichtgsten Klassen der
...
\item[basetypes] Alle definierten Komponententypen werden hier als
  Hashtabelle abgelegt. Die Komponententypen werden über ihre Namen
  angesprochen. Die Elemente sind Instanzen der Klasse
  \texttt{basetype}.
...
\item[merge-to-rules] sind die \textsf{merge-to}-Regeln. Sie sind
  Instanzen von \texttt{index-rule}.
...
\item[layers] beinhaltet die Liste der Ebenen der Lokationsstruktur.
